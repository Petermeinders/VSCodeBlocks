...
<Shared bind:this={common} />
  <main id={treeItem.id} class="single-block cube" style="background:#2c39575c; z-index:10; min-width:161px; min-height: 65px; position:absolute; {"width:" + treeItem.imageWidth + "px;"} {"height:" + treeItem.imageHeight + "px;"} { treeItem.locationX !== "0" && treeItem.locationY !== "0" ? 'transform: translate(' + treeItem.locationX + 'px' + ',' + treeItem.locationY + 'px);' : ''}">
